    ...
    if(!isConnected){
      ...
      const fetchUserLoans=async()=>{
        ...
        try{
          ...
          const result=await microLoansContract.getUserRequestedLoans(walletAddress)
          let userLoan=[];
          res.data.map(loan=>{
            userLoan.push({...loan, type:result[loan.loanIndex].typeOfLoan, description:result[loan.loanIndex].description})
          })
...
// interestRate
  const sendMoney=async(loan)=>{
    ...
    const tx=await microLoansContract.approveLoan(loan.address, loan.loanIndex, 18, Math.floor(loan.percentage), {
    ...
    <div className="bg-gray-900 min-h-screen">
      ...
      <div className="px-6 py-12">
        ...
        <div className="space-y-6 mx-10">
          {userLoans && userLoans.length!=0 ?<>{userLoans.map((loan, index) => (
            <div key={loan._id} className="bg-gray-700 p-6 rounded-lg shadow-lg">
              ...
              <div className="mt-4">
                <h3 className="text-lg font-semibold text-white">Number of Bids: {loan.bids.length}</h3>
                <h3 className="text-lg font-semibold text-white">Current Interest Rate: {loan.percentage}%</h3>
               {loan.bids.length>0 &&  <h3 className="text-lg font-semibold text-white">Last Bid By: {loan.bids[loan.bids.length-1].bidBy}</h3>}
              </div>
            </div>
        ...
        <div className="space-y-6 mx-10">
          {userApprovedBids && userApprovedBids.length!=0 ?<>{userApprovedBids.map((loan, index) => (
            <div key={loan._id} className="bg-gray-700 p-6 rounded-lg shadow-lg">
              <div className="flex justify-between items-center">
                <div>
              ...
              <div className="mt-4">
                <h3 className="text-lg font-semibold text-white">Interest Rate: {loan.percentage}%</h3>
                <h3 className="text-lg font-semibold text-white">Borrower: {loan.address}</h3>
              </div>
            </div>
          ))}</>:<h1 className="text-white">No Pending Loans</h1> }
